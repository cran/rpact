...
## |  Creation date: 14 December 2021, 13:06:46
...
test_that("'getSimulationEnrichmentSurvival': gMax = 2", {
	...
	# @refFS[Formula]{fs:simulationEnrichmentSurvivalGenerate}
...
test_that("'getSimulationEnrichmentSurvival': gMax = 3", {
	...
	expect_equal(simResult1$conditionalPowerAchieved[3, ], c(0.55391468, 0.55749541, 0.54642151, 0.56972631), tolerance = 1e-07)
	if (isTRUE(.isCompleteUnitTestSetEnabled())) {
	    invisible(capture.output(expect_error(print(simResult1), NA)))
	    expect_output(print(simResult1)$show())
	    invisible(capture.output(expect_error(summary(simResult1), NA)))
	...
	expect_equal(simResult2$conditionalPowerAchieved[3, ], c(0.50063383, 0.54643545, 0.52718133, 0.50178726), tolerance = 1e-07)
	if (isTRUE(.isCompleteUnitTestSetEnabled())) {
	    invisible(capture.output(expect_error(print(simResult2), NA)))
	    expect_output(print(simResult2)$show())
	    invisible(capture.output(expect_error(summary(simResult2), NA)))
	...
	expect_equal(simResult3$conditionalPowerAchieved[3, ], c(0.55208818, 0.54329749, 0.5548491, 0.59570521), tolerance = 1e-07)
	if (isTRUE(.isCompleteUnitTestSetEnabled())) {
	    invisible(capture.output(expect_error(print(simResult3), NA)))
	    expect_output(print(simResult3)$show())
	    invisible(capture.output(expect_error(summary(simResult3), NA)))
...
test_that("'getSimulationEnrichmentSurvival': comparison of base and enrichment for inverse normal and Fisher combination", {
	...
	x1 <- getSimulationEnrichmentSurvival(design,
	    populations = 1,
	...
	expect_equal(comp2[2, ], c(-0.022566214, -0.0056751238, 0.047207777, 0.035251356, 0.033740719, -0.051453144, 0.039406427, 0.0072692293, -0.022722897), tolerance = 1e-07)
	expect_equal(comp2[3, ], c(0.025359012, -0.021253383, 0.092581664, -0.080566447, 0.087298305, -0.050787115, 0.070673698, 0.019777739, -0.019114098), tolerance = 1e-07)
	...
	expect_equal(comp3, c(5.6713988, 8.8976118, -9.7670182, -2.0326561, -2.7081522, -0.88153524, -5.5780096, 3.3199537, 1.2334371), tolerance = 1e-07)
