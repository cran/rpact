...
.calcMeansVariancesTestStatistics <- function(
		dataInput, subset, stage, thetaH0, stratifiedAnalysis, varianceOption) {
	...
	if (stratifiedAnalysis) {
		...
		if (varianceOption == "pooledFromFull") {
			...
			testStatistics <- sum((dataInput$getMeans(stage = stage, subset = subset, group = 1) - 
				...
				na.rm = TRUE) / sqrt(sum(pv * weights, na.rm = TRUE))
			...
			testStatistics <- sum((dataInput$getMeans(stage = stage, subset = subset, group = 1) - 
				dataInput$getMeans(stage = stage, subset = subset, group = 2) - thetaH0) * weights, 
				na.rm = TRUE) / sqrt(sum(pv * weights^2, na.rm = TRUE))
		}
...
.getStageResultsMeansEnrichment <- function(..., design, dataInput,
	...
	.warnInCaseOfUnknownArguments(functionName = ".getStageResultsMeansEnrichment", 
	...
	if ((gMax > 2) && intersectionTest == "SpiessensDebois") {
		stop(C_EXCEPTION_TYPE_ILLEGAL_ARGUMENT, "gMax (", gMax ,
	...
	if (intersectionTest == "SpiessensDebois" && varianceOption != "pooledFromFull" && !normalApproximation) {
		stop("Spiessens & Depois t test can only be performed with pooled residual (stratified) variance from full population,
						 select 'varianceOption' = \"pooledFromFull\"", call. = FALSE)
	}
	
	if (intersectionTest == "SpiessensDebois" && !stratifiedAnalysis && !normalApproximation) {
		stop("Spiessens & Depois t test can only be performed with pooled residual (stratified) variance from full population,
						select 'stratifiedAnalysis' = TRUE", call. = FALSE)
	}
	...
	.setValueAndParameterType(stageResults, "intersectionTest", intersectionTest, 
		C_INTERSECTION_TEST_ENRICHMENT_DEFAULT)
	.setValueAndParameterType(stageResults, "stratifiedAnalysis", stratifiedAnalysis, 
		C_STRATIFIED_ANALYSIS_DEFAULT)
	
	effectSizes <- matrix(rep(NA_real_, gMax * kMax), gMax, kMax)
	means1 <- matrix(rep(NA_real_, gMax * kMax), gMax, kMax)
	means2 <- matrix(rep(NA_real_, gMax * kMax), gMax, kMax)
	stDevs1 <- matrix(rep(NA_real_, gMax * kMax), gMax, kMax)
	stDevs2 <- matrix(rep(NA_real_, gMax * kMax), gMax, kMax)
	...
	dimnames(testStatistics) = list(paste("population ", 1:gMax, sep = ""), 
		paste("stage ", (1:kMax), sep = ""))
	dimnames(separatePValues) = list(paste("population ", 1:gMax, sep = ""), 
		paste("stage ", (1:kMax), sep = ""))
			...
			for (population in 1:gMax) {
				...
				if (.isTrialDesignInverseNormal(design)) {	
					combInverseNormal[population, k] <- (weightsInverseNormal[1:k] %*% 
							stats::qnorm(1 - singleStepAdjustedPValues[population,1:k])) / 
						sqrt(sum(weightsInverseNormal[1:k]^2))
				}
				else if (.isTrialDesignFisher(design)) {	
					combFisher[population, k] <- prod(singleStepAdjustedPValues[population, 
						1:k]^weightsFisher[1:k])
				}
			}
		}
...
.getAnalysisResultsMeansInverseNormalEnrichment <- function(...,
	...
	stage <- .getStageFromOptionalArguments(..., dataInput = dataInput, design = design)
	.warnInCaseOfUnknownArguments(functionName = ".getAnalysisResultsMeansInverseNormalEnrichment", 
		ignore = c(.getDesignArgumentsToIgnoreAtUnknownArgumentCheck(design, powerCalculationEnabled = TRUE), "stage"), ...)
	
	results <- AnalysisResultsEnrichmentInverseNormal(design = design, dataInput = dataInput)
	
	results <- .getAnalysisResultsMeansEnrichmentAll(results = results, design = design, dataInput = dataInput, 
...
.getAnalysisResultsMeansFisherEnrichment <- function(...,
	...
	stage <- .getStageFromOptionalArguments(..., dataInput = dataInput, design = design)
	.warnInCaseOfUnknownArguments(functionName = ".getAnalysisResultsMeansFisherEnrichment", 
		ignore = c(.getDesignArgumentsToIgnoreAtUnknownArgumentCheck(design, powerCalculationEnabled = TRUE), "stage"), ...)
...
.getAnalysisResultsMeansEnrichmentAll <- function(..., results, design, dataInput, intersectionTest, stage, 
		directionUpper, normalApproximation, stratifiedAnalysis, varianceOption, thetaH0, thetaH1, assumedStDevs, 
		nPlanned, allocationRatioPlanned, tolerance,
		iterations, seed) {
	
	startTime <- Sys.time()
	
	stageResults <- .getStageResultsMeansEnrichment(design = design, dataInput = dataInput, 
	...
	if (design$kMax > 1) {
		...
		startTime <- Sys.time()
		if (.isTrialDesignFisher(design)) {
			conditionalPowerResults <- .getConditionalPowerMeansEnrichment(stageResults = stageResults, 
	...
	for (k in stages) {
		...
		for (population in 1:gMax) {
			if (!is.na(stageResults$testStatistics[population, k])) {
				...
				thetaLow <- .getUpperLowerThetaMeansEnrichment(design = design, dataInput = dataInput, 
				...
				thetaUp <- .getUpperLowerThetaMeansEnrichment(design = design, dataInput = dataInput, 
				...
				repeatedConfidenceIntervals[population, 1, k] <- .getRootThetaMeansEnrichment(design = design, 
				...
				repeatedConfidenceIntervals[population, 2, k] <- .getRootThetaMeansEnrichment(design = design, 
				...
				if (k > 1 && conditionFunction(bounds[k - 1], border) & design$bindingFutility) {
					...
					if (directionUpper) {
						thetaLow <- .getUpperLowerThetaMeansEnrichment(design = design, 
							dataInput = dataInput, 
						...
						thetaUp <- .getUpperLowerThetaMeansEnrichment(design = design, 
							dataInput = dataInput, 
					...
					futilityCorr[k] <- .getRootThetaMeansEnrichment(design = design, dataInput = dataInput, 
					...
					if (directionUpper) {
						repeatedConfidenceIntervals[population, 1, k] <- min(min(futilityCorr[2:k]), 
							repeatedConfidenceIntervals[population, 1, k])
					} else {
						repeatedConfidenceIntervals[population, 2, k] <- max(max(futilityCorr[2:k]), 
							repeatedConfidenceIntervals[population, 2, k])
					}	
				}
...
.getRepeatedConfidenceIntervalsMeansEnrichmentInverseNormal <- function(..., 
	...
	return(.getRepeatedConfidenceIntervalsMeansEnrichmentAll(design = design, dataInput = dataInput, 
			normalApproximation = normalApproximation, 
			stratifiedAnalysis = stratifiedAnalysis, 
			varianceOption = varianceOption, 
			directionUpper = directionUpper, 
...
.getConditionalPowerMeansEnrichmentInverseNormal <- function(..., results, stageResults, stage,
	...
	design <- stageResults$.design
	.assertIsTrialDesignInverseNormal(design)
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalPowerMeansEnrichmentInverseNormal", 
...
.getConditionalPowerMeansEnrichmentFisher <- function(..., results, stageResults, stage,  
	...
	.assertIsTrialDesignFisher(design)
	.assertIsValidIterationsAndSeed(iterations, seed, zeroIterationsAllowed = FALSE)
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalPowerMeansEnrichmentFisher", 
		ignore = c("stage", "design", "stDevsH1"), ...)
	kMax <- design$kMax	
	gMax <- stageResults$getGMax()
	criticalValues <- design$criticalValues
		...
		if (!is.na(ctr$separatePValues[population, stage])) {
			if (gMax == 1) {
				...
				pValues <- ctr$adjustedStageWisePValues[ctr$indices[,population] == 1,][which.max(
						ctr$overallAdjustedTestStatistics[ctr$indices[,population] == 1,stage]), 1:stage]
			}	
			if (stage < kMax - 1) {		
				...
				if (result <= 0 || result >= 1) {
					...
					results$conditionalPower[population, kMax] <- 1 - stats::pnorm(stats::qnorm(result) - 
...
.getConditionalPowerLikelihoodMeansEnrichment <- function(..., stageResults, stage, 
	...
	for (i in seq(along = thetaRange)) {
		for (population in 1:gMax) {
			...
			if (.isTrialDesignInverseNormal(design)) {
				condPowerValues[j] <- .getConditionalPowerMeansEnrichmentInverseNormal(results = results,
					stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
					allocationRatioPlanned = allocationRatioPlanned, 
					thetaH1 = thetaRange[i], assumedStDevs = assumedStDevs)$conditionalPower[population, kMax]
			}
			else if (.isTrialDesignFisher(design)) {
				condPowerValues[j] <- .getConditionalPowerMeansEnrichmentFisher(results = results,
					stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
					allocationRatioPlanned = allocationRatioPlanned, 
					thetaH1 = thetaRange[i], assumedStDevs = assumedStDevs, 
					iterations = iterations, seed = seed)$conditionalPower[population, kMax]
			}
