...
#:#
#:#  *Simulation of multiarm design with continuous data*
#:# 
		...
		if (any(selectedArms[1:gMax, stage + 1], na.rm = TRUE)) {
			if (is.na(thetaH1)) {
				thetaStandardized <- max(min(overallEffects[
					selectedArms[1:gMax, stage + 1], stage] / stDevH1, na.rm = TRUE), 1e-12)
			} else {
...
.getSimulatedStageMeansMultiArm <- function(...,
	...
	simMeans <- matrix(rep(NA_real_, (gMax + 1) * kMax), gMax + 1, kMax)
	...
	for (k in 1:kMax) {
		...
		if (subjectsPerStage[gMax + 1, k] > 0) {
			simMeans[gMax + 1, k] <- rnorm(1, 0, stDev / sqrt(subjectsPerStage[gMax + 1, k]))
		}	
		...
		if (k < kMax) {
			...
			if (.isTrialDesignConditionalDunnett(design)) {
				conditionalCriticalValue[k] <- (stats::qnorm(1 - design$alpha) - 
					stats::qnorm(1 - adjustedPValues[k]) * sqrt(design$informationAtInterim)) /
...
#' for testing means in a multi-arm treatment groups testing situation. 
#'
