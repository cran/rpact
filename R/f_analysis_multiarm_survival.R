...
.getAnalysisResultsSurvivalMultiArm <- function(..., design, dataInput) {
	...
	if (.isTrialDesignInverseNormal(design)) {
		return(.getAnalysisResultsSurvivalInverseNormalMultiArm(design = design, 
			dataInput = dataInput, ...))
	}
	
	if (.isTrialDesignFisher(design)) {
		return(.getAnalysisResultsSurvivalFisherMultiArm(design = design, 
			dataInput = dataInput, ...))
	}
	
	if (.isTrialDesignConditionalDunnett(design)) {
		return(.getAnalysisResultsSurvivalConditionalDunnettMultiArm(design = design, 
			dataInput = dataInput, ...))
	}
...
.getAnalysisResultsSurvivalConditionalDunnettMultiArm <- function(..., 
	...
	results <- .getAnalysisResultsSurvivalMultiArmAll(results = results, design = design, 
...
.getAnalysisResultsSurvivalMultiArmAll <- function(..., results, design, dataInput, intersectionTest, stage, 
	...
	if (design$kMax > 1) {
		...
		startTime <- Sys.time()
		if (.isTrialDesignFisher(design)) {
			conditionalPowerResults <- .getConditionalPowerSurvivalMultiArm(stageResults = stageResults, 
...
.getStageResultsSurvivalMultiArm <- function(..., design, dataInput,
	...
	for (k in 1:stage) {
		for (g in 1:gMax) {
			effectSizes[g, k] <- exp(dataInput$getOverallLogRanks(stage = k, group = g) * 
				(1 + dataInput$getOverallAllocationRatios(stage = k, group = g)) /
				sqrt(dataInput$getOverallAllocationRatios(stage = k, group = g) * 
				dataInput$getOverallEvents(stage = k, group = g)))
			
			testStatistics[g, k] <- dataInput$getLogRanks(stage = k, group = g)  - 
				sqrt(dataInput$getEvents(stage = k, group = g))*
				sqrt(dataInput$getAllocationRatios(stage = k, group = g))/
				(1 + dataInput$getAllocationRatios(stage = k, group = g)) * log(thetaH0)
			
			overallTestStatistics[g, k] <- dataInput$getOverallLogRanks(stage = k, group = g) - 
				sqrt(dataInput$getOverallEvents(stage = k, group = g))*
				sqrt(dataInput$getOverallAllocationRatios(stage = k, group = g))/
				(1 + dataInput$getOverallAllocationRatios(stage = k, group = g)) * log(thetaH0)
		...
		for (k in stages) {
			...
			for (g in 1:gMax) {
				if (!is.na(stageResults$testStatistics[g, k])) {
					...
					thetaLow <- exp(.getUpperLowerThetaSurvivalMultiArm(design = design, dataInput= dataInput, 
						theta = -1, treatmentArm = g, stage = k, directionUpper = TRUE, intersectionTest = intersectionTest,  
						conditionFunction = conditionFunction, firstParameterName = firstParameterName, 
						secondValue = criticalValues[k])) 
					
					thetaUp <- exp(.getUpperLowerThetaSurvivalMultiArm(design = design, dataInput= dataInput, 
						theta = 1, treatmentArm = g, stage = k, directionUpper = FALSE, intersectionTest = intersectionTest,  
						conditionFunction = conditionFunction, firstParameterName = firstParameterName, 
						secondValue = criticalValues[k])) 
					
					# finding upper and lower RCI limits through root function
					repeatedConfidenceIntervals[g, 1, k] <- .getRootThetaSurvivalMultiArm(design = design, 
					...
					repeatedConfidenceIntervals[g, 2, k] <- .getRootThetaSurvivalMultiArm(design = design, 
					...
					if (k > 1 && conditionFunction(bounds[k - 1], border) & design$bindingFutility) {
						...
						if (directionUpper) {
							...
							thetaUp <- .getUpperLowerThetaSurvivalMultiArm(design = design, 
								dataInput = dataInput, 
						...
						futilityCorr[k] <- .getRootThetaSurvivalMultiArm(design = design, dataInput = dataInput, 
...
.getConditionalPowerSurvivalMultiArm <- function(..., stageResults, stage = stageResults$stage, 
	...
	if (.isTrialDesignInverseNormal(design)) {
		return(.getConditionalPowerSurvivalMultiArmInverseNormal(results = results,
			design = design, stageResults = stageResults, stage = stage, 
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, 
			thetaH1 = thetaH1, ...))
	}
	else if (.isTrialDesignFisher(design)) {
		return(.getConditionalPowerSurvivalMultiArmFisher(results = results,
			design = design, stageResults = stageResults,  stage = stage,
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, 
			thetaH1 = thetaH1, 
			iterations = iterations, seed = seed, ...))
		...
		return(.getConditionalPowerSurvivalMultiArmConditionalDunnett(results = results,
			design = design, stageResults = stageResults, stage = stage, 
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, 
			thetaH1 = thetaH1, ...))
	}
...
.getConditionalPowerSurvivalMultiArmConditionalDunnett <- function(..., results, design, stageResults, stage,  
	...
	.assertIsTrialDesignConditionalDunnett(design)
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalPowerSurvivalMultiArmConditionalDunnett", 
...
.getConditionalPowerLikelihoodSurvivalMultiArm <- function(..., stageResults, stage, 
	...
	for (i in seq(along = thetaRange)) {
		...
		for (g in (1:gMax)) {
			...
			if (.isTrialDesignInverseNormal(design)) {
				condPowerValues[j] <- .getConditionalPowerSurvivalMultiArmInverseNormal(results = results,
					design = design, stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
					allocationRatioPlanned = allocationRatioPlanned, 
					thetaH1 = thetaRange[i], ...)$conditionalPower[g, kMax]
			}
			else if (.isTrialDesignFisher(design)) {
				condPowerValues[j] <- .getConditionalPowerSurvivalMultiArmFisher(results = results,
					design = design, stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
					allocationRatioPlanned = allocationRatioPlanned, 
				...
				condPowerValues[j] <- .getConditionalPowerSurvivalMultiArmConditionalDunnett(results = results,
					design = design, stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
					allocationRatioPlanned = allocationRatioPlanned, 
					thetaH1 = thetaRange[i], ...)$conditionalPower[g, 2]
			}
