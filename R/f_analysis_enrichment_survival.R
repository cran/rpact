...
.calcSurvivalTestStatistics <- function(dataInput, subset, stage, thetaH0, stratifiedAnalysis, directionUpper = TRUE) {
	...
	return(list(overallEvents = overallEvents,
...
.getStageResultsSurvivalEnrichment <- function(..., design, dataInput,
	...
	.warnInCaseOfUnknownArguments(functionName = ".getStageResultsSurvivalEnrichment", 
	...
	if (!stratifiedAnalysis) {
		stop(C_EXCEPTION_TYPE_ILLEGAL_ARGUMENT, "Only stratified analysis can be performed for enrichment survival designs")
	}
	...
	overallEvents <- matrix(rep(NA_real_, gMax * kMax), gMax, kMax)
	...
	for (k in 1:stage) {
		for (population in 1:gMax) {
			...
			results <- .calcSurvivalTestStatistics(dataInput, subset, k, thetaH0, stratifiedAnalysis, directionUpper)
			
			effectSizes[population, k] <- thetaH0 * exp(results$overallTestStatistics * 
				(1 + results$overallAllocationRatios) / sqrt(results$overallAllocationRatios * results$overallEvents))
...
.getAnalysisResultsSurvivalEnrichment <- function(..., design, dataInput) {
	if (.isTrialDesignInverseNormal(design)) {
		return(.getAnalysisResultsSurvivalInverseNormalEnrichment(design = design, dataInput = dataInput, ...))
	}
	
	if (.isTrialDesignFisher(design)) {
		return(.getAnalysisResultsSurvivalFisherEnrichment(design = design, dataInput = dataInput, ...))
	}
...
.getAnalysisResultsSurvivalInverseNormalEnrichment <- function(..., 
	...
	stage <- .getStageFromOptionalArguments(..., dataInput = dataInput, design = design)
	.warnInCaseOfUnknownArguments(functionName = ".getAnalysisResultsSurvivalInverseNormalEnrichment", 
		ignore = c(.getDesignArgumentsToIgnoreAtUnknownArgumentCheck(design, powerCalculationEnabled = TRUE), "stage"), ...)
	
	results <- AnalysisResultsEnrichmentInverseNormal(design = design, dataInput = dataInput)
	
	results <- .getAnalysisResultsSurvivalEnrichmentAll(results = results, design = design, dataInput = dataInput, 
...
.getAnalysisResultsSurvivalFisherEnrichment <- function(..., 
	...
	.warnInCaseOfUnknownArguments(functionName = ".getAnalysisResultsSurvivalFisherEnrichment", 
		ignore = c(.getDesignArgumentsToIgnoreAtUnknownArgumentCheck(design, powerCalculationEnabled = TRUE), "stage"), ...)
...
.getAnalysisResultsSurvivalEnrichmentAll <- function(..., results, 
	...
	stageResults <- .getStageResultsSurvivalEnrichment(design = design, dataInput = dataInput, 
	...
	if (design$kMax > 1) {
		...
		startTime <- Sys.time()
		if (.isTrialDesignFisher(design)) {
			conditionalPowerResults <- .getConditionalPowerSurvivalEnrichment(stageResults = stageResults, 
...
.getRootThetaSurvivalEnrichment <- function(..., design, dataInput, treatmentArm, stage, 
		directionUpper, stratifiedAnalysis, intersectionTest, thetaLow, thetaUp, firstParameterName, secondValue, tolerance) {
...
.getUpperLowerThetaSurvivalEnrichment <- function(..., design, dataInput, theta, treatmentArm, stage, 
		directionUpper, conditionFunction, stratifiedAnalysis, intersectionTest, firstParameterName, secondValue) {
...
.getRepeatedConfidenceIntervalsSurvivalEnrichmentAll <- function(..., 
	...
	if (any(is.na(criticalValues[1:stage]))) {
		warning("Repeated confidence intervals not because ", sum(is.na(criticalValues)), 
			" critical values are NA (", .arrayToString(criticalValues), ")", call. = FALSE)
	...
	for (k in stages) {
		...
		for (g in 1:gMax) {
			if (!is.na(stageResults$testStatistics[g, k])) {
				...
				thetaLow <- exp(.getUpperLowerThetaSurvivalEnrichment(design = design, dataInput= dataInput, 
				...
				thetaUp <- exp(.getUpperLowerThetaSurvivalEnrichment(design = design, dataInput= dataInput, 
				...
				repeatedConfidenceIntervals[g, 1, k] <- .getRootThetaSurvivalEnrichment(design = design, 
				...
				repeatedConfidenceIntervals[g, 2, k] <- .getRootThetaSurvivalEnrichment(design = design, 
				...
				if (k > 1 && conditionFunction(bounds[k - 1], border) & design$bindingFutility) {
					...
					if (directionUpper) {
						...
						thetaUp <- .getUpperLowerThetaSurvivalEnrichment(design = design, 
							dataInput = dataInput, 
					...
					futilityCorr[k] <- .getRootThetaSurvivalEnrichment(design = design, dataInput = dataInput, 
...
.getRepeatedConfidenceIntervalsSurvivalEnrichmentInverseNormal <- function(..., 
	...
	return(.getRepeatedConfidenceIntervalsSurvivalEnrichmentAll(design = design, dataInput = dataInput, 
		directionUpper = directionUpper, 
		stratifiedAnalysis = stratifiedAnalysis, intersectionTest = intersectionTest,
...
.getConditionalPowerSurvivalEnrichment <- function(..., stageResults, stage = stageResults$stage, 
	...
	if ((length(thetaH1) != 1) && (length(thetaH1) != gMax)) {
		stop(C_EXCEPTION_TYPE_ILLEGAL_ARGUMENT, 
			sprintf(paste0("length of 'thetaH1' (%s) must be ",
			"equal to 'gMax' (%s) or 1"), .arrayToString(thetaH1), gMax))
	}
	
	if (.isTrialDesignInverseNormal(design)) {
		return(.getConditionalPowerSurvivalEnrichmentInverseNormal(results = results,
			design = design, stageResults = stageResults, stage = stage, 
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, 
			thetaH1 = thetaH1, ...))
	}
	else if (.isTrialDesignFisher(design)) {
		return(.getConditionalPowerSurvivalEnrichmentFisher(results = results,
			design = design, stageResults = stageResults,  stage = stage,
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, 
			thetaH1 = thetaH1, 
			iterations = iterations, seed = seed, ...))
...
.getConditionalPowerLikelihoodSurvivalEnrichment <- function(..., stageResults, stage, 
	...
	for (i in seq(along = thetaRange)) {
		...
		for (g in (1:gMax)) {
			...
			if (.isTrialDesignInverseNormal(design)) {
				condPowerValues[j] <- .getConditionalPowerSurvivalEnrichmentInverseNormal(results = results,
					design = design, stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
					allocationRatioPlanned = allocationRatioPlanned, 
					thetaH1 = thetaRange[i], ...)$conditionalPower[g, kMax]
			}
			else if (.isTrialDesignFisher(design)) {
				condPowerValues[j] <- .getConditionalPowerSurvivalEnrichmentFisher(results = results,
					design = design, stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
					allocationRatioPlanned = allocationRatioPlanned, 
					thetaH1 = thetaRange[i], 
					iterations = iterations, seed = seed, ...)$conditionalPower[g, kMax]
			}
			likelihoodValues[j] <- stats::dnorm(log(thetaRange[i]), log(stageResults$effectSizes[g, stage]),
					stdErr[g]) / stats::dnorm(0, 0, stdErr[g])
			j <- j + 1		
		}		
	}	
