	...
	if (designNumber == 1L) {
		...
		if (groups == 1) {
			if (!normalApproximation) {
				if (directionUpper) {
					value <- stats::qnorm(1 - stats::pbinom(e1 - 1, n1, thetaH0, lower.tail = FALSE)) 
				} else {
					value <- stats::qnorm(1 - stats::pbinom(e1, n1, thetaH0, lower.tail = TRUE))
				}
			} else {
			...
			if (!normalApproximation) {
				if (directionUpper) {
					value <- stats::qnorm(1 - stats::phyper(e1 - 1, e1 + e2, n1 + n2 - e1 - e2, n1, lower.tail = FALSE))
				} else {
					value <- stats::qnorm(1 - stats::phyper(e1,	e1 + e2, n1 + n2 - e1 - e2, n1,	lower.tail = TRUE)) 
				}
			} else {
				if (!riskRatio) {
					if (r1 - r2 - thetaH0 == 0) {
		...
		if (stage == 1) {
			...
			if (groups == 1) {
				if (!normalApproximation) {
					if (directionUpper) {
						testStatisticsPerStage <- stats::qnorm(1 - stats::pbinom(e1 - 1, n1, thetaH0, lower.tail = FALSE)) 
					} else {
						testStatisticsPerStage <- stats::qnorm(1 - stats::pbinom(e1, n1, thetaH0, lower.tail = TRUE))
					}
				} else {
				...
				if (!normalApproximation) {
					if (directionUpper) {
						testStatisticsPerStage <- stats::qnorm(1 - stats::phyper(e1 - 1, e1 + e2, n1 + n2 - e1 - e2, n1, lower.tail = FALSE))
					} else {
						testStatisticsPerStage <- stats::qnorm(1 - stats::phyper(e1, e1 + e2, n1 + n2 - e1 - e2, n1, lower.tail = TRUE)) 
					}
				} else {	
					if (!riskRatio) {
						if (r1 - r2 - thetaH0 == 0) {
			...
			if (groups == 1) {
				if (!normalApproximation) {
					if (directionUpper) {
						testStatisticsPerStage <- c(testStatisticsPerStage,
								stats::qnorm(1 - stats::pbinom(e1 - 1, n1, thetaH0, lower.tail = FALSE))) 
					} else {
						testStatisticsPerStage <- c(testStatisticsPerStage, 
								stats::qnorm(1 - stats::pbinom(e1, n1, thetaH0, lower.tail = TRUE)))
					}
				} else {
					testStatisticsPerStage <- c(testStatisticsPerStage, 
				...
				if (!normalApproximation) {
					if (directionUpper) {
						testStatisticsPerStage <- c(testStatisticsPerStage,
								stats::qnorm(1 - stats::phyper(e1 - 1, e1 + e2, n1 + n2 - e1 - e2, n1, lower.tail = FALSE)))
					} else {
						testStatisticsPerStage <- c(testStatisticsPerStage,
								stats::qnorm(1 - stats::phyper(e1, e1 + e2, n1 + n2 - e1 - e2, n1, lower.tail = TRUE))) 
					}
				} else {
					if (!riskRatio) {
						if (r1 - r2 - thetaH0 == 0) {
...
.getSimulationStepRates <- function(..., 
	...
	if (k > 1) {
		...
		if (groups == 2) {
			if (!riskRatio) {
				fm <- .getFarringtonManningValuesDiff(rate1 = overallRate[1], rate2 = overallRate[2], 
					theta = thetaH0, allocation = allocationRatioPlanned)
			} else {
				fm <- .getFarringtonManningValuesRatio(rate1 = overallRate[1], rate2 = overallRate[2], 
					theta = thetaH0, allocation = allocationRatioPlanned)
			}
		}	
...
getSimulationRates <- function(design = NULL, ...,
	...
	conditionalPower <- .ignoreParameterIfNotUsed("conditionalPower", 
		conditionalPower, design$kMax > 1, "design is fixed ('kMax' = 1)")
	minNumberOfSubjectsPerStage <- .ignoreParameterIfNotUsed("minNumberOfSubjectsPerStage", 
		minNumberOfSubjectsPerStage, design$kMax > 1, "design is fixed ('kMax' = 1)")
	maxNumberOfSubjectsPerStage <- .ignoreParameterIfNotUsed("maxNumberOfSubjectsPerStage", 
	...
	pi1H1 <- .ignoreParameterIfNotUsed("pi1H1", pi1H1, design$kMax > 1, "design is fixed ('kMax' = 1)")
	if (is.na(conditionalPower) && is.null(calcSubjectsFunction) && !is.na(pi1H1)) {
		warning("'pi1H1' will be ignored because neither 'conditionalPower' nor 'calcSubjectsFunction' is defined", call. = FALSE)	
	}	
	pi2H1 <- .ignoreParameterIfNotUsed("pi2H1", pi2H1, design$kMax > 1, "design is fixed ('kMax' = 1)")
	if (is.na(conditionalPower) && is.null(calcSubjectsFunction) && !is.na(pi2H1)) {
		warning("'pi2H1' will be ignored because neither 'conditionalPower' nor 'calcSubjectsFunction' is defined", call. = FALSE)	
	}	
