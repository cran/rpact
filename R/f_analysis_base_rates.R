...
#:#  Last changed: $Date: 2020-10-30 17:12:19 +0100 (Fri, 30 Oct 2020) $
...
.getAnalysisResultsRates <- function(..., design, dataInput) {
	if (.isTrialDesignGroupSequential(design)) {
		return(.getAnalysisResultsRatesGroupSequential(design = design, 
				dataInput = dataInput, ...))
	}
	
	if (.isTrialDesignInverseNormal(design)) {
		return(.getAnalysisResultsRatesInverseNormal(design = design, 
				dataInput = dataInput, ...))
	}
	
	if (.isTrialDesignFisher(design)) {
		return(.getAnalysisResultsRatesFisher(design = design, 
				dataInput = dataInput, ...))
	}
...
.getAnalysisResultsRatesInverseNormal <- function(..., design,
	...
	.getAnalysisResultsRatesAll(results = results, design = design, dataInput = dataInput, 
...
.getAnalysisResultsRatesAll <- function(..., results, design, dataInput, stage, 
	...
	if (dataInput$getNumberOfGroups() == 2) {
		...
		if (!all(is.na(pi2))) {
			warning("'pi2' (", .arrayToString(pi2), ") will be ignored ", 
				"because the specified data input has only one group", call. = FALSE)
		}
	...
	if (design$kMax > 1) {
		...
		startTime <- Sys.time()
		if (.isTrialDesignFisher(design)) {
			results$.conditionalPowerResults <- .getConditionalPowerRates(stageResults = stageResults, 
...
.getStageResultsRates <- function(..., design, dataInput, thetaH0 = NA_real_, 
	...
	.warnInCaseOfUnknownArguments(functionName = "getStageResultsRates", 
		...
		for (k in 1:stage) {
			if (normalApproximation) {
				if (thetaH0 == 0) {
					...
					y <- .getFarringtonManningValues(rate1 = overallEvents1[k] / sum(dataInput$getSampleSizesUpTo(k, 1)), 
		...
		if (dataInput$getNumberOfGroups() == 1) {
			...
			if (normalApproximation) {
				...
				if (thetaH0 == 0) {
					...
					y <- .getFarringtonManningValues(rate1 = dataInput$getEvent(k, 1)/dataInput$getSampleSize(k, 1), 
	...
	if (dataInput$getNumberOfGroups() == 1) {
		...
		stageResults <- StageResultsRates(
			...
			overallPValues = .fillWithNAs(overallPValues, design$kMax), 
			effectSizes = effectSizes,
			overallEvents1 = .fillWithNAs(dataInput$getOverallEventsUpTo(stage, group = 1), design$kMax),
			overallEvents2 = .fillWithNAs(dataInput$getOverallEventsUpTo(stage, group = 2), design$kMax),
			overallSampleSizes1 = .fillWithNAs(dataInput$getOverallSampleSizesUpTo(stage, 1), design$kMax),
...
.getRepeatedConfidenceIntervalsRatesGroupSequential <- function(..., design, dataInput,   
		normalApproximation = C_NORMAL_APPROXIMATION_RATES_DEFAULT, directionUpper = C_DIRECTION_UPPER_DEFAULT, 
		tolerance = C_ANALYSIS_TOLERANCE_DEFAULT) {
	...
	return(.getRepeatedConfidenceIntervalsRatesAll(design = design, dataInput = dataInput, 
...
#
.getConditionalPowerRatesGroupSequential <- function(..., stageResults, stage = stageResults$stage,  
	...
	if (stageResults$isTwoSampleDataset()) {
		x <- .getFarringtonManningValues(rate1 = pi1, rate2 = pi2, theta = stageResults$thetaH0, 
...
#
.getConditionalPowerRatesInverseNormal <- function(..., stageResults, stage = stageResults$stage, 
		nPlanned, allocationRatioPlanned = C_ALLOCATION_RATIO_DEFAULT, pi1, pi2) {
	...
	if (stageResults$isTwoSampleDataset()) {
		x <- .getFarringtonManningValues(rate1 = pi1, rate2 = pi2, 
			theta = stageResults$thetaH0, allocation = allocationRatioPlanned)
		adjustment <- stats::qnorm(1 - condError) * (1 - 
			sqrt(x$ml1 * (1 - x$ml1) + allocationRatioPlanned * x$ml2 * (1 - x$ml2)) / 
			sqrt(pi1 * (1 - pi1) + allocationRatioPlanned * pi2 * (1 - pi2))) * 
...
.getConditionalPowerRates <- function(..., stageResults, nPlanned, 
	...
	results <- ConditionalPowerResultsRates(.stageResults = stageResults, 
	...
	results$.setParameterType("pi1", C_PARAM_DEFAULT_VALUE)
	results$.setParameterType("pi2", C_PARAM_DEFAULT_VALUE)
			...
			for (i in seq(along = piTreatmentRange)) {
				if (.isTrialDesignGroupSequential(design)) {
					condPowerValues[i] <- .getConditionalPowerRatesGroupSequential(..., 
						stageResults = stageResults, nPlanned = nPlanned, 
						allocationRatioPlanned = allocationRatioPlanned, 
						pi1 = piTreatmentRange[i], pi2 = pi2)$conditionalPower[design$kMax]
				}
	...
	if (!is.na(finalStage)) {
		...
		if (stageGroupSeq == 1) {
			...
			finalConfidenceInterval[1] <- .getRootThetaRates(
				...
				firstParameterName = "overallPValues", secondValue = stats::qnorm(1 - design$alpha/design$sided), 
...
.getFinalConfidenceIntervalRatesInverseNormal <- function(..., design, dataInput, stage, 
	...
	if (!is.na(finalStage)) {
		...
		if (stageInverseNormal == 1) {
			...
			finalConfidenceInterval[1] <- .getRootThetaRates(
				design = design, dataInput = dataInput, stage = 1, 
				directionUpper = C_DIRECTION_UPPER_DEFAULT, normalApproximation = T, 
				firstParameterName = "combInverseNormal", 
				secondValue = stats::qnorm(1 - design$alpha/design$sided), tolerance = tolerance,
				acceptResultsOutOfTolerance = TRUE,
				callingFunctionInformation = "Final confidence interval [1]")
...
.getFinalConfidenceIntervalRates <- function(..., design, dataInput, 
	...
	stage <- .getStageFromOptionalArguments(..., dataInput = dataInput, design = design)
	.assertIsValidThetaH0DataInput(thetaH0, dataInput)
	.warnInCaseOfUnknownArguments(functionName = "getFinalConfidenceIntervalRates", 
		ignore = c(.getDesignArgumentsToIgnoreAtUnknownArgumentCheck(design), "stage"), ...)
