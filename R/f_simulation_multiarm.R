...
#			}	
	...
	return(list(separatePValues	= separatePValues, 
...
.getCriticalValuesDunnettForSimulation <- function(alpha, indices, allocationRatioPlanned) {
	...
	for (i in 1:(2^gMax - 1)) {
		...
		sigma <- zeta %*% t(zeta)
		diag(sigma) <- 1
		criticalValuesDunnett[i] <- .getMultivariateDistribution(type = "quantile", 
...
.createSimulationResultsMultiArmObject <- function(..., 
	...
	if (endpoint == "means") {
		effectMatrix <- .assertIsValidEffectMatrixMeans(typeOfShape = typeOfShape, effectMatrix = effectMatrix, 
		...
		.setValueAndParameterType(simulationResults, "piControlH1", piControlH1, C_PI_2_DEFAULT)
		
		effectMatrix <- .assertIsValidEffectMatrixRates(typeOfShape = typeOfShape, effectMatrix = effectMatrix, 
	...
	if (endpoint %in% c("means", "rates")) { # means + rates only
		...
		minNumberOfEventsPerStage <- .ignoreParameterIfNotUsed("minNumberOfEventsPerStage", 
		...
		maxNumberOfEventsPerStage <- .ignoreParameterIfNotUsed("maxNumberOfEventsPerStage", 
	...
	if (endpoint %in% c("means", "rates") && is.na(conditionalPower) && is.null(calcSubjectsFunction)) {
		if (length(minNumberOfSubjectsPerStage) != 1 ||	!is.na(minNumberOfSubjectsPerStage)) {
			warning("'minNumberOfSubjectsPerStage' (", 
				...
				"neither 'conditionalPower' nor 'calcSubjectsFunction' is defined", call. = FALSE)
	...
	if (endpoint == "means") {
		...
		if (is.null(calcEventsFunction)) {
			...
			.assertIsValidFunction(fun = calcEventsFunction, 
	...
	if (endpoint %in% c("means", "rates")) {
		...
		.setValueAndParameterType(simulationResults, "plannedEvents", plannedEvents, NA_real_)		
		.setValueAndParameterType(simulationResults, "minNumberOfEventsPerStage", 
			minNumberOfEventsPerStage, NA_real_, notApplicableIfNA = TRUE)
		.setValueAndParameterType(simulationResults, "maxNumberOfEventsPerStage", 
			maxNumberOfEventsPerStage, NA_real_, notApplicableIfNA = TRUE)
	}
