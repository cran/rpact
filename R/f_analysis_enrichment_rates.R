...
.calcRatesTestStatistics <- function(dataInput, subset, stage, thetaH0, stratifiedAnalysis, normalApproximation, directionUpper) {
	...
	if (!all(is.na(dataInput$getSampleSizes(stage = stage, subset = subset)))) {
		...
		if (stratifiedAnalysis) {
			...
			if (thetaH0 == 0) {
				if (sum(actEv + ctrEv, na.rm = TRUE) == 0 || sum(actEv + ctrEv, na.rm = TRUE) == sum(actN + ctrN, na.rm = TRUE)) {
					testStatistics <- 0
				} else {
				...
				for (g in (1:length(subset))) {
					y <- .getFarringtonManningValues(rate1 = actEv[g]/ actN[g], 
						rate2 = ctrEv[g] / ctrN[g], theta = thetaH0, allocation = actN[g] / ctrN[g], method = "diff")
					actMl[g] <- y$ml1
					ctrMl[g] <- y$ml2
				}
				testStatistics <- sum((actEv / actN - ctrEv / ctrN - thetaH0) * weights, na.rm = TRUE) /
		...
		# non-stratified analysis
			...
			if (normalApproximation) {
				if (thetaH0 == 0) {
					...
					y <- .getFarringtonManningValues(rate1 = e[1] / n[1], 
...
.getStageResultsRatesEnrichment <- function(..., design, dataInput,
	...
	.warnInCaseOfUnknownArguments(functionName = ".getStageResultsRatesEnrichment", 
	...
	if (intersectionTest == "SpiessensDebois" && !normalApproximation) {
		stop(C_EXCEPTION_TYPE_ILLEGAL_ARGUMENT, 
			"Spiessens & Debois test cannot be used with Fisher's exact test (normalApproximation = FALSE)", call. = FALSE)
	}
	...
	if (stratifiedAnalysis && !dataInput$isStratified()) {
		stop(C_EXCEPTION_TYPE_ILLEGAL_ARGUMENT, "Stratified analysis is only possible for stratified data input")
	}
	...
	for (k in 1:stage) {
		for (population in (1:gMax)) {
			...
			results <- .calcRatesTestStatistics(dataInput, subset, k, thetaH0, stratifiedAnalysis, normalApproximation, directionUpper)
...
.getAnalysisResultsRatesInverseNormalEnrichment <- function(...,
	...
	stage <- .getStageFromOptionalArguments(..., dataInput = dataInput, design = design)
	.warnInCaseOfUnknownArguments(functionName = ".getAnalysisResultsRatesInverseNormalEnrichment", 
		ignore = c(.getDesignArgumentsToIgnoreAtUnknownArgumentCheck(design, powerCalculationEnabled = TRUE), "stage"), ...)
	
	results <- AnalysisResultsEnrichmentInverseNormal(design = design, dataInput = dataInput)
	
	results <- .getAnalysisResultsRatesEnrichmentAll(results = results, design = design, dataInput = dataInput, 
...
.getAnalysisResultsRatesFisherEnrichment <- function(...,
	...
	stage <- .getStageFromOptionalArguments(..., dataInput = dataInput, design = design)
	.warnInCaseOfUnknownArguments(functionName = ".getAnalysisResultsRatesFisherEnrichment", 
		ignore = c(.getDesignArgumentsToIgnoreAtUnknownArgumentCheck(design, powerCalculationEnabled = TRUE), "stage"), ...)
...
.getAnalysisResultsRatesEnrichmentAll <- function(..., results, design, dataInput, 
	...
	stageResults <- .getStageResultsRatesEnrichment(design = design, dataInput = dataInput, 
	...
	if (results$.getParameterType("piControls") %in% c(C_PARAM_TYPE_UNKNOWN, C_PARAM_NOT_APPLICABLE)) {
		.setValueAndParameterType(results, "piControls", 
	...
	if (design$kMax > 1) {
		...
		startTime <- Sys.time()
		if (.isTrialDesignFisher(design)) {
			conditionalPowerResults <- .getConditionalPowerRatesEnrichment(stageResults = stageResults, 
	...
	for (k in 1:design$kMax) {
		for (population in 1:gMax) {
			results$repeatedConfidenceIntervalLowerBounds[population, k] <- repeatedConfidenceIntervals[population, 1, k]
			results$repeatedConfidenceIntervalUpperBounds[population, k]  <- repeatedConfidenceIntervals[population, 2, k]
		}
	}
	...
	for (k in stages) {
		...
		for (g in 1:gMax) {
			if (!is.na(stageResults$testStatistics[g, k])) {
				...
				repeatedConfidenceIntervals[g, 1, k] <- .getRootThetaRatesEnrichment(design = design, 
					dataInput = dataInput, population = g, stage = k, directionUpper = TRUE, 
					normalApproximation = normalApproximation, 
					stratifiedAnalysis = stratifiedAnalysis,
				...
				repeatedConfidenceIntervals[g, 2, k] <- .getRootThetaRatesEnrichment(design = design, 
					dataInput = dataInput, population = g, stage = k, directionUpper = FALSE, 
					normalApproximation = normalApproximation, 
					stratifiedAnalysis = stratifiedAnalysis,
					thetaLow = thetaLow, thetaUp = thetaUp, 
					intersectionTest = intersectionTest, firstParameterName = firstParameterName, 
				...
				if (k > 1 && conditionFunction(bounds[k - 1], border) & design$bindingFutility) {
					...
					futilityCorr[k] <- .getRootThetaRatesEnrichment(design = design, dataInput = dataInput, 
						population = g, stage = k - 1, directionUpper = directionUpper, 
						normalApproximation = normalApproximation, 
						stratifiedAnalysis = stratifiedAnalysis,
						thetaLow = thetaLow, thetaUp = thetaUp, 
...
.getRepeatedConfidenceIntervalsRatesEnrichmentInverseNormal <- function(..., 
	...
	return(.getRepeatedConfidenceIntervalsRatesEnrichmentAll(design = design, dataInput = dataInput, 
...
.getConditionalPowerRatesEnrichment <- function(..., stageResults, stage = stageResults$stage	, 
	...
	if (!is.null(piTreatmentsH1) && !is.na(piTreatmentsH1)) {
		if (!is.na(piTreatments)) {
			warning(sQuote("piTreatments"), " will be ignored because ", sQuote("piTreatmentsH1"), " is defined", call. = FALSE)
		}
	...
	if (!is.null(piControlH1) && !is.na(piControlH1)) {
		if (!is.na(piControl)) {
			warning(sQuote("piControl"), " will be ignored because ", sQuote("piControlH1"), " is defined", call. = FALSE)
		}
	...
	if ((length(piControls) != 1) && (length(piControls) != gMax)) {
		stop(C_EXCEPTION_TYPE_ILLEGAL_ARGUMENT, 
			sprintf(paste0("length of 'piControls' (%s) ",
			"must be equal to 'gMax' (%s) or 1"), .arrayToString(piControls), gMax))
	}
	
	if (.isTrialDesignInverseNormal(design)) {
		return(.getConditionalPowerRatesEnrichmentInverseNormal(results = results,
		...
		return(.getConditionalPowerRatesEnrichmentFisher(results = results,
			design = design, stageResults = stageResults,  stage = stage,
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, 
...
.getConditionalPowerRatesEnrichmentInverseNormal <- function(..., results, design, stageResults, stage,
	...
	.assertIsTrialDesignInverseNormal(design)
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalPowerRatesEnrichmentInverseNormal",
...
.getConditionalPowerRatesEnrichmentFisher <- function(..., results, design, stageResults, stage,  
	...
	.assertIsTrialDesignFisher(design)
	.assertIsValidIterationsAndSeed(iterations, seed, zeroIterationsAllowed = FALSE)
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalPowerRatesEnrichmentFisher",
		ignore = c("piTreatmentsH1", "piControlH1"), ...)
	...
	if (useAdjustment) {
		condError <- .getConditionalRejectionProbabilitiesEnrichment(design = design, stageResults = stageResults, 
...
.getConditionalPowerLikelihoodRatesEnrichment <- function(..., stageResults, stage, 
	...
	for (i in seq(along = piTreatmentRange)) {
		for (g in (1:gMax)) {
			...
			if (.isTrialDesignInverseNormal(design)) {
				condPowerValues[j] <- .getConditionalPowerRatesEnrichmentInverseNormal(results = results,
				...
				condPowerValues[j] <- .getConditionalPowerRatesEnrichmentFisher(results = results,
					design = design, stageResults = stageResults, stage = stage, nPlanned = nPlanned, 
			...
			likelihoodValues[j] <- stats::dnorm(piTreatmentRange[i], stageResults$overallPisTreatment[g, stage], stdErr[g]) / 
