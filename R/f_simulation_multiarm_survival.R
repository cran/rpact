		...
		if (any(selectedArms[1:gMax, stage + 1], na.rm = TRUE)) {
			if (is.na(thetaH1)) {
				if (directionUpper) {
					thetaStandardized <- log(max(min(
						overallEffects[selectedArms[1:gMax, stage + 1], stage], na.rm = TRUE), 1 + 1e-07))
				} else {
			...
			if (conditionalCriticalValue[stage] > 8) {
				...
				newEvents <- (1 + allocationRatioPlanned)^2 / allocationRatioPlanned *
					...
					stats::qnorm(conditionalPower), na.rm = TRUE))^2 / thetaStandardized^2
				newEvents <- min(max(minNumberOfEventsPerStage[stage + 1], newEvents), 
