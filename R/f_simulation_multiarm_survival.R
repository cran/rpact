...
#:# 
#:#  File version: $Revision: 3823 $
#:#  Last changed: $Date: 2020-11-03 11:43:15 +0100 (Di, 03 Nov 2020) $
		...
		for (treatmentArm in 1:gMax) {
			if (selectedArms[treatmentArm, k]) {
				if (k == 1) {
					...
					#new:
					eventsPerStage[treatmentArm, k] <- plannedEvents[k] * 
							(allocationRatioPlanned * omegaVector[treatmentArm] + 1) / (allocationRatioPlanned * sum(omegaVector) + 1)
				} else {
					#eventsPerStage[treatmentArm, k] <- plannedEvents[k] - plannedEvents[k - 1]
					#new:
					eventsPerStage[treatmentArm, k] <- (plannedEvents[k] - plannedEvents[k - 1]) * 
							(allocationRatioPlanned * omegaVector[treatmentArm] + 1) / (allocationRatioPlanned * sum(omegaVector[selectedArms[, k]]) + 1)
				}	
