...
.getTestStatisticsMeans <- function(..., designNumber, informationRates, groups, normalApproximation, 
	...
	if (normalApproximation) {
		...
		pValuesSeparate <- 1 - pt(testStatisticsPerStage, sampleSizesPerStage - groups)
	}	
	...
	if (designNumber == 1L) {
		...
		if (normalApproximation) {
			...
			value <- prod((1 - pt(testStatisticsPerStage[1:stage], sampleSizesPerStage[1:stage] - groups))^weightsFisher[1:stage])
		}
	}	
...
.getSimulationStepMeans <- function(..., 
	...
	testStatistic <- .getTestStatisticsMeans(designNumber = designNumber, 
...
getSimulationMeans <- function(
	...
	stDevH1 <- .ignoreParameterIfNotUsed("stDevH1", stDevH1, design$kMax > 1, "design is fixed ('kMax' = 1)", "Assumed effect")
	...
	.setValueAndParameterType(simulationResults, "stDevH1", 
		stDevH1, NA_real_, notApplicableIfNA = TRUE)
