...
.getAnalysisResultsMultiArm <- function(
	...
	if (dataInput$isDatasetMeans()) {
		...
		return(.getAnalysisResultsMeansMultiArm(design = design, 
	...
	if (dataInput$isDatasetRates()) {
		...
		return(.getAnalysisResultsRatesMultiArm(design = design, dataInput = dataInput, intersectionTest = intersectionTest, 
			directionUpper = directionUpper, thetaH0 = thetaH0, nPlanned = nPlanned, ...))
	}
	...
	if (dataInput$isDatasetSurvival()) {
		...
		return(.getAnalysisResultsSurvivalMultiArm(design = design, dataInput = dataInput, intersectionTest = intersectionTest,  
			directionUpper = directionUpper, thetaH0 = thetaH0, nPlanned = nPlanned, ...))
	}
	...
	if (stageResults$isDatasetMeans()) {
		...
		if ("assumedStDev" %in% names(list(...))) {
			warning("For multi-arm analysis the argument for assumed standard deviation ",
				"is named 'assumedStDevs' and not 'assumedStDev'", call. = FALSE)
		}
		
		return(.getConditionalPowerMeansMultiArm(stageResults = stageResults,
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, ...))
	}

	if (stageResults$isDatasetRates()) {
		return(.getConditionalPowerRatesMultiArm(stageResults = stageResults,
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, ...))
	}
	
	if (stageResults$isDatasetSurvival()) {
		return(.getConditionalPowerSurvivalMultiArm(stageResults = stageResults,
			nPlanned = nPlanned, allocationRatioPlanned = allocationRatioPlanned, ...))
	}
...
.getIndicesOfClosedHypothesesSystemOld1 <- function(gMax) {
...
.getIndicesOfClosedHypothesesSystemOld2 <- function(gMax) {
	indices <- as.matrix(expand.grid(rep(list(1:0), gMax)))[1:(2^gMax - 1), ]
	if (gMax == 1) {
		return(as.matrix(indices))
	} 
	indices <- cbind(indices, as.numeric(rowSums(indices)))
	indices <- indices[order(indices[,gMax + 1], decreasing = TRUE), ]
	indices <- indices[, 1:(ncol(indices) - 1)]
	return(indices)
}
...
#' 
#' @description
#' Calculates and returns the results from the closed combination test.
#'  
	...
	if (.isStageResultsMultiArmMeans(stageResults)) {
		if (stageResults$intersectionTest == "Dunnett" && stageResults$varianceOption != "overallPooled" &&	
			...
			warning("'varianceOption' was set to \"overallPooled\" because ",
				"Dunnett t test can only be performed with overall variance estimation", call. = FALSE)
			stageResults$varianceOption <- "overallPooled"
		}
	}
...
.getClosedConditionalDunnettTestResults <- function(..., 
	...
	for (i in 1:(2^gMax - 1)) {
		...
		sigma <- zeta %*% t(zeta)
		diag(sigma) <- 1
		crit <- .getMultivariateDistribution(type = "quantile", 
			...
			if (!all(is.na(stageResults$separatePValues[indices[i, ] == 1, 2]))) {
				...
				if (secondStageConditioning) {
					maxOverallTestStatistic <- max(
							signedOverallTestStatistics[indices[i, ] == 1, 2], na.rm = TRUE)
					integrand <- function(x) {
						innerProduct <- 1
						for (g in (1:gMax)) {
...
.getConditionalDunnettTestForCI <- function( ..., design, stageResults, treatmentArm) {
	...
	if (stageResults$directionUpper) {
		signedTestStatistics <- stageResults$testStatistics
		signedOverallTestStatistics <- stageResults$overallTestStatistics
		signedOverallTestStatistics[,2] <- sqrt(informationAtInterim)*stageResults$testStatistics[, 1] + sqrt(1 - informationAtInterim)*stageResults$testStatistics[,2] 
	} else {
		signedTestStatistics <- -stageResults$testStatistics 
		signedOverallTestStatistics <- -stageResults$overallTestStatistics
		signedOverallTestStatistics[,2] <- -(sqrt(informationAtInterim)*stageResults$testStatistics[, 1] + sqrt(1 - informationAtInterim)*stageResults$testStatistics[,2]) 
...
.getConditionalRejectionProbabilitiesMultiArmInverseNormal <- function(..., stageResults, stage) {
	...
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalRejectionProbabilitiesMultiArmInverseNormal", 
...
.getConditionalRejectionProbabilitiesMultiArmFisher <- function(..., stageResults, stage) {
	...
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalRejectionProbabilitiesMultiArmFisher", 
			ignore = c("stage", "design"), ...)
	kMax <- design$kMax
	if (kMax == 1) {
		return(as.matrix(NA_real_))
			...
			if (!is.na(stageResults$separatePValues[g, stageIndex])) {
				if (gMax == 1) {
					...
					pValues <- ctr$adjustedStageWisePValues[ctr$indices[,g] == 1,][which.max(
									ctr$overallAdjustedTestStatistics[ctr$indices[,g] == 1,stageIndex]), 1:stageIndex]
				}	
				if (prod(pValues^weights[1:stageIndex]) <= criticalValues[stageIndex]) {
					conditionalRejectionProbabilities[g,stageIndex] <- 1
				} else {
					if (stageIndex < kMax - 1) {
						conditionalRejectionProbabilities[g,stageIndex] <- .getFisherCombinationSize(kMax - stageIndex, 
...
.getConditionalRejectionProbabilitiesMultiArmConditionalDunnett <- function(..., stageResults) {
	...
	design <- stageResults$.design
	.assertIsTrialDesignConditionalDunnett(design)
	.warnInCaseOfUnknownArguments(functionName = ".getConditionalRejectionProbabilitiesMultiArmConditionalDunnett", 
